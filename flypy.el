;;; flypy.el  -*- lexical-binding: t; -*-
(require 'cl-lib)
(require 'flypy-cn)
(require 'flypy-en)
(require 'flypy-system)
(require 'flypy-macos)
;; (require 'flypy-asim)

(cond
 ((eq system-type 'darwin) (setq flypy-system-switch-chinese 'flypy-macos-switch-chinese
				 flypy-system-switch-english 'flypy-macos-switch-english))
 ((eq system-type 'windows-nt) (setq flypy-system-switch-chinese 'flypy-macos-switch-chinese
				     flypy-system-switch-english 'flypy-macos-switch-english))
 ((eq system-type 'gnu/linux) (setq flypy-system-switch-chinese 'flypy-macos-switch-chinese
				    flypy-system-switch-english 'flypy-macos-switch-english)))

(flypy-system-register)

;;;###autoload
(defun flypy-edit-flypy-cn ()
  (interactive)
  (find-file (locate-library "flypy-cn"))
  (read-only-mode -1))


;; (set-input-method "chinese-system")
;; (set-input-method "english-system")
;; (set-input-method "chinese-flypy")

;; 如果是用 set-input-method 命令的,可以正常关闭 system input method
;; 但是如果是手动打开的就关闭不了.
(defun flypy-set-input-method (name)
  "防止是手动切换的系统输入法,导致在 emacs 中千幻输入法的时候不能复原手动对系统输入法的修改.
还可以把 capslock 切换输入法的命令调用 emacs 中的输入法切换命令"
  (set-input-method "english-system")
  (run-with-timer 0.05 nil (lambda () (set-input-method name))))

;; (flypy-set-input-method "chinese-system")
(defun flypy-disable-input-method (&rest _)
  "disable 所有输入法,主要是 evil normal"
  (interactive)
  (flypy-set-input-method nil))
;; (add-hook 'evil-normal-state-entry-hook 'flypy-disable-input-method)

(defun flypy-enable ()
  ;; (flypy-system-register)
  (add-hook 'evil-normal-state-entry-hook 'flypy-disable-input-method)
  (advice-add 'evil-normal-state :before 'flypy-disable-input-method)
  )

(defun flypy-disable ()
  (remove-hook 'evil-normal-state-entry-hook 'flypy-disable-input-method)
  (set-input-method nil)
  (advice-remove 'evil-normal-state 'flypy-disable-input-method)
  )

;;;###autoload
(define-minor-mode flypy-mode
  "input method minor mode"
  :lighter " flypy"
  :global t
  (cond
   (flypy-mode (flypy-enable))
   (t (flypy-disable))))
...
;; ;; (completing-read "im: " (list "quail" "pyim" "system"))"quail
;; (defun flypy-toggle-system-im ()
;;   (memq current-input-method '("english-system" "chinese-system"))

;;   )
;; (defun enim-p ()
;;   (string= current-input-method enim)
;;   )
;; (defun flypy-set-im (im)
;;   (interactive
;;    (list (intern (completing-read "im: "
;; 				  '(quail pyim system)))))
;;   (setq flypy-default-im
;; 	(pcase im
;; 	  ('quail
;; 	   `())))
;;   )
...
;; ;;;; 设置中文，英文输入法
;; (defalias 'flypy-macos-sogou-chinese 'flypy-macos-switch-sogou)
;; (setq flypy-macos-cnim 'flypy-macos-switch-sogou)
;; (setq flypy-macos-enim 'flypy-macos-switch-english)

;; ;;都使用的是 quail 框架的,好统一处理
;; (setq flypy-quail-cnim "chinese-flypy")
;; (setq flypy-quail-enim "english-flypy")
...
;; Value:
;; (("english-flypy" "English" quail-use-package "En" "flypy-en")
;;  ("chinese-flypy" "Chinese" quail-use-package "鹤" "flypy")
;;  ("pyim" "euc-cn" pyim-start "灵通")

;;  (register-input-method "english-system" "English" 'flypy-macos-switch-english "Sen" "系统")
;;  (register-input-method "chinese-system" "Chinese" 'flypy-macos-switch-chinese "Scn" "系统中文输入法")
;;  (functionp  (nth 2 (assoc "chinese-system" input-method-alist)))
;;  ("chinese-system" "Chinese" flypy-macos-switch-sogou "Scn" "系统中文输入法")
;; (activate-input-method "chinese-system")
;; (functionp 'flypy-macos-switch-chinese)
;; ;; (defun flypy-quail-disable ()
...
;; ddd dddddddddd的drugoㄉh一heehㄏㄏ
;; ;; (setq flypy-system-cnim )
;; ;; (setq flypy-system-enim )
;; ;; (setq flypy-pyim)
...
;; ;;;; 设置无输入法状态时的光标
;; (defun set-noim-cursor ()
;;   (set-cursor-color noim-cursor-color)
;;   ;; (setq cursor-type '(bar . 5))
;;   (setq cursor-type '(hbar . 5))
;;   )
...
;; (defalias 'deactivate-flypy 'set-noim)

;; ;;;; 设置当前输入法为中文,quail框架下的中文输入
...
;;   (interactive)
;;   ;; (when (featurep 'flypy) (require 'flypy))
;;   (if
...
;; (defalias 'activate-flypy 'toggle-cnim-enim)
...
;; ;;   (interactive)
;; ;;   (let* ((pos (point))
;; ;;          (len (nthcdr 3 (syntax-ppss pos))))
;; ;;     (if (= len 0)
;; ;;         (if (pyim-string-match-p "\"" (str-before-0))
;; ;;             t
;; ;;           nil)
;; ;;       t)))
...
;; (defun point-in-string-p ()
;;   (interactive)
;;   (car (nthcdr 3 (syntax-ppss (point))))
;;   )
;; ;;;; org mode code
;; ;;;;; 前面是block
;; ;; (org-in-block-p)
...
;; ;; (add-hook 'evil-insert-state-entry-hook #'flypy/insert-mode-with-im)

;; (defun flypy/insert-mode-with-im ()
...
;; ;;                               (enim-only-mode -1)
;; ;;                               ))
;; (defun flypy/enim-only-mode-off ()
;;   (interactive)
;;   (enim-only-mode -1)
;;   )
...
;; ;; 默认 o 后切换到中文输入
;; ;; 就是光标前面是 * .
;; (general-def flypy-mode-map
;;   :predicate 'evil-org-mode
;;   :states 'normal
;;   "o" (lambda (count)
;;         (interactive "P")
...
;; ;;; flypy-mode
;; (define-minor-mode flypy-mode
;;   "flypy input method  mode."
;;   ;; :global t
;;   :lighter " flypy"
;;   :keymap (let ((map (make-sparse-keymap)))
;;             (define-key map (kbd "C-\\") 'toggle-cnim-enim)
;;             (define-key map (kbd "SPC") 'flypy-space-switch)
;;             (define-key map (kbd "DEL") 'flypy/del)
;;             map)
;;   ;; :variable flypy-mode
;;   (cond ((and flypy-mode (featurep 'flypy))
;;          (add-hook 'evil-normal-state-entry-hook (lambda () (set-input-method enim)))
;;          (add-hook 'post-self-insert-hook 'default-enim)
;;          (add-hook 'helm-minibuffer-set-up-hook 'set-enim)
;;          (advice-add 'quail-self-insert-command :before #'auto-toggle-input-method)
;;          (add-hook 'evil-insert-state-entry-hook #'flypy/insert-mode-with-im)
...
;;         (t
;;          (advice-remove 'quail-self-insert-command  #'auto-toggle-input-method)
;;          (remove-hook 'evil-normal-state-entry-hook (lambda () (set-input-method enim)))
;;          (remove-hook 'post-self-insert-hook 'default-enim)
;;          (remove-hook 'helm-minibuffer-set-up-hook 'set-enim)
;;          (remove-hook 'evil-insert-state-entry-hook #'flypy/insert-mode-with-im)
;;          (set-noim)
;;          )
;;         )
;;    )

;; (add-hook 'org-mode-hook 'flypy-mode)
;; (imap flypy-mode-map "SPC" 'flypy-space-switch)
;; (imap flypy-mode-map "DEL" 'flypy/del)
;; (gmap "C-\\" (lambda () (interactive) (flypy-mode 1) (toggle-cnim-enim)))

;; (add-hook 'minibuffer-setup-hook (lambda () (interactive) (flypy-mode -1)))
